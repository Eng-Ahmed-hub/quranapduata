        ...
        .then(data=>{
           ...
           for(let i = 0 ; i < numberOfSurahs ; i++){
            ...
            surhasContainers.innerHTML +=
             `
                <div class="suhas">
                        <p>${suharas[i].name.long}</p> 
                        <p>${suharas[i].name.transliteration.en}</p> 
                 </div>
             `
           }
           ...
           surahaTitels.forEach((title,index)=>{
            title.addEventListener('click',()=>{
                 fetch(`https://api.quran.sutanlab.id/surah/${index + 1}`)  //  الغلط في المنطقه دي ي هندسه بيطلعلي الصفحه فاضيه 
                 .then(response => response.json())
                 .then(data=>{
                    ...
                    verses.forEach(verse=>{
                        ...
                        ayatContainer.innerHTML += `
                           ...
                           <p>(${verse.number.inSurah}) - ${verse.text.arab}</p> 
                        `
                        // السطر اللي فوق دا الغلط كان فيه ابقي حد الخاصية كابي بست من الapi   
                        //كمان انت كنت كاتب verses  مش verse  مفرد 
                    })
                 })
            })
